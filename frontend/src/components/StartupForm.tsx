  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await Promise.all([
        ...
        generateSWOTAnalysis(formData),
        generateLegalChecklist(formData),
        generateFailurePrediction(formData),
      ]);
      const localFormData = JSON.parse(localStorage.getItem("formData"));
      const localFailure = JSON.parse(
        localStorage.getItem("failure-prediction"),
      );
      const localLegal = JSON.parse(localStorage.getItem("legal-checklist"));
      const localRoadmap = JSON.parse(localStorage.getItem("roadmap"));
      const localSwot = JSON.parse(localStorage.getItem("swot"));
      const newRecentRoadmap = {
        formData: localFormData,
        failurePrediction: localFailure,
        roadmap: localRoadmap,
        swotAnalysis: localSwot,
        legalChecklist: localLegal,
      };
      const recentRoadmaps =
        JSON.parse(localStorage.getItem("recent-roadmaps")) || [];

      // Add the new roadmap to the beginning of the array (most recent first)
      recentRoadmaps.unshift(newRecentRoadmap);

      // Optional: Limit the list to last 5 entries
      if (recentRoadmaps.length > 5) {
        recentRoadmaps.pop();
      }

      // Save updated list back to localStorage
      localStorage.setItem("recent-roadmaps", JSON.stringify(recentRoadmaps));
      if (response[0]) {
    ...
    <div className="max-w-4xl mx-auto">
      <Card className="glass-panel border-none shadow-lg animate-scale-in">
        <CardHeader>
          ...
          <CardDescription>
            Tell us about your startup to generate a personalized roadmap and
            insights
          </CardDescription>
        </CardHeader>
        <CardContent>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                  ...
                  <div className="space-y-2">
                    ...
                    <Select
                      ...
                      onValueChange={(value) =>
                        updateFormData("marketSize", value)
                      }
                    >
                  ...
                  <div className="space-y-2">
                    ...
                    >
                      ...
                      <SelectContent>
                        {getRegionsForCountry(formData.country).map(
                          (region) => (
              ...
              >
                ...
                <div className="space-y-2">
                  ...
                  <Textarea
                    ...
                    onChange={(e) =>
                      updateFormData("targetCustomer", e.target.value)
                    }
                  />
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <Textarea
                    ...
                    onChange={(e) =>
                      updateFormData("uniqueValueProposition", e.target.value)
                    }
                  />
                </div>
